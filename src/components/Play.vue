  ...
  mounted() {
    ...
    function ProgressTime(options) {
      this.options = {
        ...
        startTime: new Date(
          formatDate(
            new Date(new Date().getTime() - dayTimestamp * 10),
            "YYYY/MM/DD 00:00:00"
          )
        ), // 开始时间：2000/10/20 00:00:00
    ...
    ProgressTime.prototype.createDom = function () {
      ...
      var right = $(
        "<div class='" + this.id + "-right " + isRight + "'></div>"
      );

      var leftHtml =
        "<div class='" +
        this.id +
        "-left-t'></div><div class='" +
        this.id +
        "-left-b'><span class='" +
        this.id +
        "-left-b-start'></span></div>";
      ...
      var centerHtml =
        ...
        "-center-t'><div class='" +
        this.id +
        "-center-t-bar'></div></div><div class='" +
        this.id +
        "-center-c'></div><div class='" +
        this.id +
        "-center-b'></div>";
    ...
    ProgressTime.prototype.createCenter = function () {
      ...
      cTop.append(
        "<div class='" +
          this.id +
          "-center-t-tooltip'></div><div class='" +
          this.id +
          "-center-t-tooltipTemp hide'></div>"
      );
      var days =
        (this.endTime.getTime() - this.startTime.getTime()) / dayTimestamp;
      ...
      for (var i = 0; i < days; i++) {
        ...
        cCenterHtml +=
          "<li class='" +
          this.id +
          "-center-c-ul-li' style='width:" +
          width +
          ";'></li>";
        cBottomHtml += "<li style='width:" + width + ";'>" + tempTime + "</li>";
      }
      cTop.append(
        "<ul class='" + this.id + "-center-t-ul'>" + cTopHtml + "</ul>"
      );
      cCenter.append(
        "<ul class='" + this.id + "-center-c-ul'>" + cCenterHtml + "</ul>"
      );
      cbottom.append(
        "<ul class='" + this.id + "-center-b-ul'>" + cBottomHtml + "</ul>"
      );

      // 按3小时来间隔
      var spanHtml = "";
      var liWidth = $(
        "." + this.id + "-center-c-ul-li"
      )[0].getBoundingClientRect().width; //
      var hoursWidth = liWidth / 24; // 计算每个小时所占的宽度
      var intervalCount = 24 / this.options.hoursInterval;
      for (var i = 1; i < intervalCount; i++) {
        var temp =
          ((hoursWidth * i * this.options.hoursInterval) / liWidth) * 100 + "%";
        spanHtml +=
          "<span style='left:" +
          temp +
          ";'>" +
          i * this.options.hoursInterval +
          "</span>";
      }
    ...
    ProgressTime.prototype.setConfig = function (config) {
      ...
      var widthPercent =
        (config.width / $("." + this.id + "-center-t").width()) * 100 + "%"; // 用百分比，解决时间播放结束出现自适应问题
      // 灵活的tooltip
      if (config.type === "mousemove") {
        $("." + this.id + "-center-t-tooltipTemp")
          .removeClass("hide")
          .text(text)
          .css({
            left: widthPercent,
        ...
        .animate(
          ...
          function () {
            ...
            typeof _this.options.animateCallback === "function" &&
              _this.options.animateCallback(config);
          }
        );
    ...
    ProgressTime.prototype.setTime = function (e) {
      ...
      var type = e && e.type;
      var hoursWidth =
        $("." + this.id + "-center-t-ul li")[0].getBoundingClientRect().width /
        24; // 计算每个小时所占的宽度
      var tooltipTimestamp = this.currentTimeTemp || this.currentTime.getTime(); // 返回当前的时间的时间戳
      var num = Math.floor(
        (tooltipTimestamp - this.startTime.getTime()) / hourTimestamp
      ); // 计算出多少小时
      ...
      if (layerX) {
        // 移动或点击重新求距离
        tooltipTimestamp = Math.floor(
          this.startTime.getTime() + num * hourTimestamp
        );
    ...
    ProgressTime.prototype.bindEvent = function () {
      ...
      $("." + this.id + "-left-b-start").on("click", function () {
        ...
        if (flag && _this.currentTimeTemp < _this.endTime) {
          _this.currentTimeTemp = _this.currentTimeTemp
            ? _this.currentTimeTemp
            : _this.currentTime.getTime(); // 加一小时的时间戳
          flag = false;
          self.addClass("stop");
          if (!_this.timer) {
            _this.timer = setInterval(function () {
              if (_this.options.toPlay && _this.options.animateFinish) {
                // 动画完成后，并且页面渲染后再调用
                console.log("aaa");
                _this.options.toPlay = false;
                _this.options.animateFinish = false;
                _this.currentTimeTemp += hourTimestamp;
                if (_this.currentTimeTemp >= _this.endTime) {
                  // 时间边界判定
                  flag = true;
    ...
    ProgressTime.prototype.resize = function () {
      // 自适应只调用一次，解决浏览器触发多次resize方法
      var _this = this;
      var tempTimer = null;
      $(window).resize(function () {
        if (!tempTimer && !_this.timer) {
          // 浏览器缩放会重置时间轴，这里判断_this.timer是否存在再重置
    ...
    $("#paly-container").ProgressTime({
      container: "progressTime",
      startTime: new Date(
        window.formatDate(
          new Date(new Date().getTime() - dayTimestamp * 5),
          "YYYY/MM/DD 00:00:00"
        )
      ),
      ...
      currentTime: new Date(
        window.formatDate(
          new Date(new Date().getTime() - dayTimestamp * 4),
          "YYYY/MM/DD 12:00:00"
        )
      ),
      interval: 300,
